...
import json
from models.visual_language_model import vllm_predict, vllm_predict_in_json, generate_base_prompt, debug_vllm_with_history, debug_vllm_generate_KB
...
def run_model(img_path, input_text, system_prompt, vllm_info_model, llm_info_model):
    ...
    if vllm_info_model == 'accounts/fireworks/models/llama-v3p2-90b-vision-instruct':
        vllm_response, vllm_history_to_llm, vllm_token_used = debug_vllm_generate_KB(image_base64, trans_input_text, system_prompt, vllm_info_model)
    ...
    total_token_used = vllm_token_used
    str_cost = 'Rp. %.6f' % total_cost
    
    stop = timeit.default_timer()
    time_process = "{:0,.2f}".format(stop - start) 
    # logger.info(f'Time process: {stop - start}')
    return trans_input_text, vllm_response,  vllm_token_used, total_token_used, str_cost, time_process
