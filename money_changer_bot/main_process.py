...
from commons.llm_instructions import LLM_step_by_step_template, LLM_instruct_currency_converter, Instruct_CoT_1_LLM, generate_CoT_2_LLM
...
def run_llm_model_CoT(vllm_response, text_input, llm_info_model):
    ...
    prompt_CoT_1 = llm_input + '\nRephrase and expand the question, and respond. ' + Instruct_CoT_1_LLM
    CoT_1_llm_response = client.chat.completions.create(
        model=llm_info_model,
        response_format={"type": "json_object"},
        messages=[{"role": "user", "content": prompt_CoT_1}],
        temperature=0,
    )
    #--Reuseable content
    # base_response = CoT_1_llm_response.choices[0].message.content
    # msg = [
    #         {"role": "system", "content": f'You are a helpful assistant.' + intro_Any_to_USD_currency}]
    # msg = [{"role": "assistant", "content": base_response}]
    # msg.append({"role": "user", "content": f'{Instruct_CoT_2_LLM}'})
    ...
    final_price =compute_conversion(base_response['source_exchange_rate'], base_response['value'], base_response['target_exchange_rate'])
    llm_token_used = (CoT_1_llm_response.usage.total_tokens + CoT_2_llm_response.usage.total_tokens )
    cost = ((0.2 /1e6) * llm_token_used )* 15125.90
    combine_llm_response = CoT_1_llm_response.choices[0].message.content + '\n\n\n' + 'Reformat to json: ' + CoT_2_llm_response.choices[0].message.content
...
def run_model(img_path, input_text, vllm_info_model, llm_info_model):
    ...
    # llm_response, final_response, llm_token_used, llm_cost = run_llm(vllm_response, trans_input_text, llm_info_model)
